...
#### Methods for event binding with this keyword -

- Use bind keyword and bind the handler in the render method.

- Using arrow function in render method. Calling the event handler in arrow function body.  
  Disadvantage of method 1 & 2: Every update to the state will cause the component to re-render. So it is not ideal in cases where a large component is being updated or if it has lots of nested components. Impact can become severe in large/nested components.  
  Advantage: Simpler to use and pass parameters.

- Binding event handler in constructor instead of render method.  
...
class EventBind extends Component {
  ...
  /* 
    Method 4: Using arrow function as a class property.
      Basically we are changing the way to define the method in class i.e. Using arrow function instead of function keyword
  */
  clickHandler = () => {
    this.setState({
      message: "Shutting down ! dun dun dun dun",
  ...
  render() {
    return (
      <div>
        ...
        {/* Method 2: Using arrow function in the render method, here we are using arrow function to run the clickHandler() method (calling the eventHandler method and returning the value), so we are using parenthesis and we can omit return keyword after => as there is a single return statement */}
